...
def preprocess_images(images, labels):
    ...
    print('Done generating images, ' + str(time.time()-start))
...
def generate_images(datagen, images, labels):
    ...
    generated_images = np.empty( (0,np.size(images,axis = 1), 
                                  np.size(images,axis = 2), np.size(images,axis = 3)),
    float)
    generated_labels = np.empty( (0,np.size(labels,axis = 1)), float)
